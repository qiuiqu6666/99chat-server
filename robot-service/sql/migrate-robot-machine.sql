...
-- Usage: mysql -h127.0.0.1 -ujiqiren -p jiqiren < sql/migrate-robot-machine.sql
--
-- player_group_id in snapshot/daily/runtime/tasks becomes semantically = machine_code.
-- Existing player_group_id values are seeded as ACTIVE machines so sync can continue
-- after cutover once Windows sends X-Machine-Code equal to that id (or re-registers
-- and rebinds groups). Bindings start empty — each IM group must 配对/开启 again.
...
CREATE TABLE IF NOT EXISTS robot_group_binding (
    ...
    KEY idx_robot_group_machine (machine_code),
...
-- Seed one machine row per existing tenant key so historical data remains addressable.
