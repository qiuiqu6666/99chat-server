...
import { listRecharges } from "@/api/finance";
    ...
    <div class="page-card">
      ...
      <el-table :data="rows" v-loading="loading" stripe border>
        <el-table-column label="用户" min-width="140">
          <template #default="{ row }"><UserUidLink :uid="str(row, 'user_uid', 'userUid', 'from_uid', 'fromUid', 'sender_uid', 'senderUid')" /></template>
        </el-table-column>
        <el-table-column label="单号" min-width="140"><template #default="{ row }">{{ str(row, 'biz_no', 'bizNo', 'order_no') || '—' }}</template></el-table-column>
        <el-table-column label="昵称" min-width="100"><template #default="{ row }">{{ row.nickname || '—' }}</template></el-table-column>
        <el-table-column label="渠道" width="100"><template #default="{ row }">{{ row.channel || '—' }}</template></el-table-column>
        ...
        <el-table-column label="地址" min-width="160" show-overflow-tooltip><template #default="{ row }">{{ row.address || '—' }}</template></el-table-column>
