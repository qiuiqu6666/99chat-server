...
/**
 * AI 助手统一入口：外部只访问主服务，主服务把 /ai-assistant/** 原样转发到内网 ai-assistant-service。
 * 例：POST /ai-assistant/api/v1/chat/stream → http://127.0.0.1:8095/api/v1/chat/stream。
 */
@ConditionalOnProperty(name = "ai-assistant.proxy-enabled", havingValue = "true", matchIfMissing = true)
@RestController
public class AiAssistantServiceProxyController {

    private static final Logger log = LoggerFactory.getLogger(AiAssistantServiceProxyController.class);

    private static final String PREFIX = "/ai-assistant";
    ...
    private final String aiAssistantServiceUrl;

    public AiAssistantServiceProxyController(
            @Value("${ai-assistant.service-url:http://127.0.0.1:8095}") String aiAssistantServiceUrl) {
        this.aiAssistantServiceUrl = aiAssistantServiceUrl.endsWith("/")
            ? aiAssistantServiceUrl.substring(0, aiAssistantServiceUrl.length() - 1)
    ...
    public ResponseEntity<StreamingResponseBody> proxy(HttpServletRequest request) throws IOException {
        ...
        URI target = URI.create(aiAssistantServiceUrl + path + (query != null ? "?" + query : ""));
        boolean sse = path.startsWith("/api/v1/chat/stream");
        ...
        if ("GET".equals(method) || "HEAD".equals(method)) {
            ...
            byte[] body = AiAssistantRequestBodies.read(request);
        ...
        try {
            ...
            log.error("ai-assistant-service proxy failed path={} target={} reason={}", path, target, e.toString());
            byte[] msg = "{\"ok\":false,\"error\":\"ai-assistant service unavailable\"}"
